...
namespace MyNotes
{
	...
	public class DocContent
	{
		...
		/// <summary>Sets or Get the type of Content of the Document.</summary>
		public DocTypes DocType { get; set; } = 0;
		...
		public AccountSecurity Security { get; set; } = null;
		...
		public override string ToString()
		{
			return String.Format("'{0}': {1} childs.", Title, (Childs.Count > 0 ? Childs.Count.ToString() : "No"));
		}
	}
	...
	/// <summary>To Store Account information in a Document's Body, with optional Security.</summary>
	public class AccountManager
	{
		public AccountManager() { }

		//public AccountSecurity Security { get; set; } = new AccountSecurity();

		/// <summary>Data of the Stored Accounts.</summary>
		public List<Account> Accounts { get; set; } = new List<Account>();

		public List<Account> FindAccount(string pFilter)
		{
			List<Account> _ret = null;
			try
			{
				//TODO
			}
	...
	public class AccountSecurity
	{
		...
		{
			get => !string.IsNullOrEmpty(_plain_password) ? Convert.ToBase64String(Encoding.UTF8.GetBytes(_plain_password)) : string.Empty;
			set => _plain_password = !string.IsNullOrEmpty(value) ? Encoding.UTF8.GetString(Convert.FromBase64String(value)) : string.Empty;
		}
		...
		[JsonIgnore]
		/// <summary>Determines if Authentication Errors will be Raized.</summary>
		public bool ThrowAuthErrors { get; set; } = true;

		/// <summary>Returns 'true' if the user has already validaded the password in thise session.</summary>
		...
		[JsonIgnore]
		/// <summary>'true' if a Password is set.</summary>
		public bool IsSecured { get => !string.IsNullOrEmpty(EncryptedPassword); }
		...
		/// <summary>Validates the inpunt password against the current one, if exists.</summary>
		/// <param name="pPassord">Inpunt password to Validate</param>
		/// <returns>'true' if the password is valid.</returns>
		public bool Validate(string pPassord)
		{
			_isValidated = _plain_password == pPassord;
			if (!_isValidated && ThrowAuthErrors) throw new Exception("Incorrect Password!");
			return _isValidated;
		}

		/// <summary>Try to Assign a new Password.
		/// Si no hay contraseña, se asigna la nueva.
		/// Si ya habia contraseña, se debe Validar.
		/// </summary>
		/// <param name="pNewPassword">The New Password</param>
		/// <param name="pOldPassord">Old password for verification</param>
		/// <returns>'true' if the password was succesfully changed.</returns>
		public bool SetPassword(string pNewPassword, string pOldPassord)
		{
			bool _ret = false;
			try
			{
				if (string.IsNullOrEmpty(_plain_password))
				{
					_plain_password = pNewPassword;
					_ret = true;
				}
				else
				{
					if (Validate(pOldPassord))
					{
						_plain_password = pNewPassword;
						_ret = true;
					}
		...
		/// <summary>Removes the Current Password.</summary>
		/// <param name="pPassord">Current password for verification</param>
		/// <returns>'true' if the password was succesfully changed.</returns>
		public bool RemovePassword(string pPassord)
		{
			bool _ret = false;
			try
			{
				if (!string.IsNullOrEmpty(_plain_password))
				{
					if (Validate(pPassord))
					{
						_plain_password = string.Empty;
						_ret = true;
					}
				}
			}
	...
	{
		public Account() { }
		public Account(string pName, string pEmail, string pPassword, string pUser = "", string pExtra = "")
		{
			Name = pName;
			Email = pEmail;
			Password = pPassword;
	...
	public static class Util
	{
		...
		/// <summary>Abre un archivo JSON devolviendo su contenido como una cadena de Texto.</summary>
		/// <param name="filePath">Ruta completa al archivo que se quiere leer.</param>
		/// <param name="Beautify">[OPCIONAL] 'true' devuelve el JSON formateado.</param>
		/// <returns></returns>		
		public static string JSON_ReadFile(string filePath, bool Beautify = true)
		{
			string _ret = string.Empty;
			try
			{
				...
				{
					// Open the file in shared mode, allowing read access for multiple processes
					using (FileStream fs = File.Open(filePath, FileMode.Open, FileAccess.Read, FileShare.Read))
					{
						using (TextReader reader = new StreamReader(fs, Encoding.UTF8))
						{
							_ret = reader.ReadToEnd();
							_ret = JsonConvert.SerializeObject(Newtonsoft.Json.Linq.JObject.Parse(_ret),
								Beautify ? Formatting.Indented : Formatting.None);
						}
					}
				}
			}
		...
		public static bool JSON_WriteFile(string filePath, string jsonData, bool Beautify = true)
		{
			try
			{
				...
				// Open the file for writing, allowing shared access for reading
				using (FileStream fs = File.Open(filePath, FileMode.Create, FileAccess.Write, FileShare.Read))
				{
					...
					{
						// Serialize the JSON data with optional formatting
						string formattedJson = JsonConvert.SerializeObject(jsonData,
							Beautify ? Formatting.Indented : Formatting.None);
		...
		public static string Serialize_ToJSON<T>(T objectToWrite, bool Beautify = true) where T : new()
		{
			string _ret = string.Empty;
			try
			{
		...
		[DllImport("DwmApi")] //System.Runtime.InteropServices, Win32API to set Windows Dark Theme:
		...
		public static DialogResult InputBox(string title, string promptText, ref List<KeyValue> Fields,
			Base64Icons.MsgIcon Icon = 0, string ButtonsTexts = "Aceptar|Cancelar|Si|No|Continuar")
		{
			Form form = new Form
			{
			...
			System.Drawing.Color ColorsControl = SystemColors.Control;
			System.Drawing.Color ColorsControlText = SystemColors.ControlText;
			System.Drawing.Color ColorsPanel = SystemColors.ControlLightLight;
			System.Drawing.Color ColorsHightlight = Color.FromArgb(0, 99, 177);

			bool IsDarkMode = (GetWindowsColorMode() <= 0); //<- O: DarkMode, 1: LightMode
			if (IsDarkMode)
			{
				//Apply Window's Dark Mode to the Form's Title bar
				form.HandleCreated += (object sender, EventArgs e) =>
				{
					if (DwmSetWindowAttribute(form.Handle, 19, new[] { 1 }, 4) != 0)
						DwmSetWindowAttribute(form.Handle, 20, new[] { 1 }, 4);

					//Scrollbars:
					DwmSetWindowAttribute(form.Handle, 1029, new[] { 1 }, 4);
				};

				ColorsControl = Color.FromArgb(32,32,32);
				ColorsPanel = Color.FromArgb(45,45,48);
				ColorsControlText = Color.FromArgb(238, 238, 238);
				form.BackColor = ColorsControl;
				form.ForeColor = ColorsControlText;
			}
			...
			Panel bottomPanel = new Panel
			{
				...
				Height = 48,
				BackColor = ColorsPanel,
				ForeColor = ColorsControlText
			};
			form.Controls.Add(bottomPanel);

			//string base64_Info = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\info-circle.png");
			//string base64_Success = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\success-tick.png");
			//string base64_Warning = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\Warning.png");
			//string base64_Error = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\error-delete.png");
			//string base64_Question = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\question.png");
			//string base64_Lock = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\lock-open.png");
			//string base64_User = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\user-circle.png");
			//string base64_Forbidden = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\forbidden.png");
			//string base64_AddNew = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\add-circle.png");
			//string base64_Cancel = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\cancel.png");
			//string base64_Edit = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\edit-circle.png");
			//string base64_List = Util.OpenImageBase64(@"C:\Users\Jhollman\Documents\CUTCSA\Iconos\items-list.png");
			...
			if (Icon != Base64Icons.MsgIcon.None)
			{
				...
				picIcon.SetBounds(0, 0, 48, 48);
				picIcon.BringToFront();
			}

			var palabras = ButtonsTexts.Split('|'); //<- "OK|Cancel|Yes|No|Continue"
			...
			buttonCancel.FlatAppearance.BorderColor = ColorsControl;
			buttonOk.FlatAppearance.BorderColor = ColorsHightlight;			

			bottomPanel.Controls.Add(buttonOk);
			bottomPanel.Controls.Add(buttonCancel);

			buttonOk.SetBounds(form.ClientSize.Width - 77, (bottomPanel.Height - buttonOk.Height) / 2, 75, 23);
			buttonCancel.SetBounds(buttonOk.Left - 77, (bottomPanel.Height - buttonOk.Height) / 2, 75, 23);
			...
			foreach (KeyValue field in Fields)
			{
				...
				if (field.ValueType == KeyValue.ValueTypes.Date)
				{
					field_Control = new DateTimePicker
					{
						...
						CalendarForeColor = ColorsControlText,
						CalendarMonthBackground = ColorsControl,
						CalendarTitleBackColor = ColorsPanel,
						CalendarTitleForeColor = ColorsControlText,						
					};
		...
		public static Image ConvertBase64ToImage(string base64Data)
		{
			try
			{
				...
				//Image bitmap = (Image)new System.Drawing.ImageConverter().ConvertFrom(imageBytes);

				using (MemoryStream ms = new MemoryStream(imageBytes))
				{
					// Ensure proper stream position before loading image
					ms.Position = 0;
			...
			{
				// Handle potential errors like invalid Base64 data, unsupported formats, or corruption
				Console.WriteLine("Error converting Base64 to Image: {0}", ex.Message);
		...
		/// <summary>Codifica un Texto usando Base64.</summary>
		/// <param name="plainText">Cadena de Texo a Codificar</param>
		public static string Base64_Encode(string plainText)
		{
			var plainTextBytes = System.Text.Encoding.UTF8.GetBytes(plainText);
			return System.Convert.ToBase64String(plainTextBytes);
		}
		/// <summary>Decodifica una cadena desde Base64 hacia Texto Simple.</summary>
		/// <param name="base64EncodedData">Texto en Base64</param>
		...
		/// <summary>Verifica si una Cadena de texto ha sido codificada en Base64.</summary>
		/// <param name="Text">Cadena a verificar</param>
		/// <returns>'true' si la cadena contiene codigo Base64 Válido.</returns>
		public static bool Base64_IsEncoded(string Text)
		{
			try
			{
		...
		/// <summary>Codifica un Texto usando el algoritmo Triple-DES.</summary>
		/// <param name="toEncrypt">Texto a Codificar</param>
		/// <param name="useHashing">If hashing use get hashcode regards to your key</param>
		/// <param name="key">Clave a Usar</param>
		public static string TrileDES_Encrypt(string toEncrypt, bool useHashing, string key)
		{
			byte[] keyArray;
			...
			//string key = "cutcsa"; // (string)settingsReader.GetValue("SecurityKey", typeof(String));
			...
			ICryptoTransform cTransform = tdes.CreateEncryptor();       //transform the specified region of bytes array to resultArray
			byte[] resultArray =
		...
		/// <summary>DeCodifica un texto usando el algoritmo Triple-DES.</summary>
		/// <param name="cipherString">Texto a De-Codificar</param>
		/// <param name="useHashing">if hashing was used get the hash code with regards to your key</param>
		/// <param name="key">Clave a Usar</param>
		public static string TrileDES_Decrypt(string cipherString, bool useHashing, string key)
		{
			byte[] keyArray;
			byte[] toEncryptArray = Convert.FromBase64String(cipherString); //get the byte code of the string
		...
		private static byte[] AES_Decrypt(byte[] bytesToBeDecrypted, byte[] passwordBytes)
		{
			...
			byte[] saltBytes = new byte[] { 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8 };
			using (MemoryStream ms = new MemoryStream())
			{
				using (RijndaelManaged AES = new RijndaelManaged())
				{
		...
		public static string AES_EncryptText(string TextInput, string password)
		{
			byte[] bytesToBeEncrypted = Encoding.UTF8.GetBytes(TextInput);
			...
			//Los bytes encriptados se convierten ahora a Base64:
			string result = Convert.ToBase64String(bytesEncrypted);
			return result;
		}
		public static string AES_DecryptText(string TextInput, string password)
		{
			byte[] bytesToBeDecrypted = Convert.FromBase64String(TextInput); //El texto encriptado es una cadena Base64
			byte[] passwordBytes = Encoding.UTF8.GetBytes(password);
			passwordBytes = SHA256.Create().ComputeHash(passwordBytes);
		...
		/// <summary>Genera una clave de 64 bits para poder encriptar  archivos.</summary>
		...
		/// <summary>Abre el archivo sInputFilename, lo encripta y lo escribe como sOutputFilename</summary>
		...
		/// <summary>Abre el archivo sInputFilename  lo desencripta con sKey y lo reescribe  como sOutputFilename</summary>
		/// <param name="sInputFilename"></param>
		/// <param name="sOutputFilename"></param>
		/// <param name="sKey"></param>
		public static void DencryptFile(string sInputFilename, string sOutputFilename, string sKey)
		{
			DESCryptoServiceProvider DES = new DESCryptoServiceProvider();
			...
			BinaryReader reader = new BinaryReader(cryptostreamDecr);
